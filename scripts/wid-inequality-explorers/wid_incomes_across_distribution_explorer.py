...
# This code creates the tsv file for the inequality explorer from the WID data, available [here](https://owid.cloud/admin/explorers/preview/wid-keymetrics)
...
for tab in range(len(tables)):
    ...
    for wel in range(len(welfare)):
        ...
        for dec9 in range(len(deciles9)):
            df_tables.loc[
                j, "name"
            ] = f"{deciles9['ordinal'][dec9].capitalize()} - Threshold ({welfare['technical_text'][wel].capitalize()})"
            df_tables.loc[
                j, "slug"
        ...
        for top in range(len(top_pct)):
            df_tables.loc[
                j, "name"
            ] = f"{top_pct['name'][top].capitalize()} - Share ({welfare['technical_text'][wel].capitalize()})"
            df_tables.loc[
                j, "slug"
