...
df_tables.loc[
    df_tables["colorScaleNumericMinValue"].isnull(), "colorScaleNumericMinValue"
...
df_tables["colorScaleNumericMinValue"] = df_tables["colorScaleNumericMinValue"].astype(
    "Int64"
)
...
for tab in range(len(tables)):
    for agg in range(len(income_aggregation)):
        for wel in range(len(welfare)):
            ...
            df_graphers.loc[j, "Period Radio"] = (
            ...
            df_graphers.loc[j, "note"] = (
                f"This data is expressed in international-$ at {PPP_YEAR_WID} prices. {welfare['note'][wel]}"
            )
        ...
        df_graphers.loc[j, "note"] = (
            f"This data is expressed in international-$ at {PPP_YEAR_WID} prices."
        )
        df_graphers.loc[j, "selectedFacetStrategy"] = "entity"
        df_graphers.loc[j, "hasMapTab"] = "false"
        df_graphers.loc[j, "tab"] = "chart"
